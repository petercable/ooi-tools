...
from util.common import log_timing, TimeRange, FUNCTION, to_xray_dataset, StreamKey
...
def build_stream_dictionary():
    d = {}
    for row in _get_stream_metadata():
        d.setdefault(row.method, {}).setdefault(row.stream, []).append((row.subsite, row.node, row.sensor, row.method))
    return d
